...
# Background training launcher for physical NPU card 11.
...
# Kill stale train_spetr processes (WARNING: also kills card10 if running).
...
export WORK_DIR_BASE=/mnt/sfs_turbo/workdir/wfc1/work_dirs_card11_${DATE_TAG}
mkdir -p "$WORK_DIR_BASE" /mnt/sfs_turbo/workdir/wfc1/diagnostics

nohup ./run_train.sh "$CONFIG" > /mnt/sfs_turbo/workdir/wfc1/diagnostics/train_card11_${DATE_TAG}.log 2>&1 &

echo "card11 training started in background"
echo "PID: $!"
echo "work_dir: $WORK_DIR_BASE"
echo "launcher log: /mnt/sfs_turbo/workdir/wfc1/diagnostics/train_card11_${DATE_TAG}.log"
