...
def multi_scale_deformable_attn_pytorch(
    ...
    """CPU version of multi-scale deformable attention with NPU fallback to CPU.

    The NPU backward of F.grid_sample is non-deterministic even across repeated
    calls on the same physical card. Routing NPU inputs through CPU makes the
    forward/backward bitwise deterministic, which is required for cross-card
    alignment verification.
    """
    if value.device.type == 'npu':
        original_device = value.device
        output = _multi_scale_deformable_attn_pytorch_ref(
            value.cpu(), value_spatial_shapes.cpu(),
            sampling_locations.cpu(), attention_weights.cpu())
        return output.to(original_device)
    return _multi_scale_deformable_attn_pytorch_ref(
        value, value_spatial_shapes, sampling_locations, attention_weights)

def _multi_scale_deformable_attn_pytorch_ref(
        value: torch.Tensor, value_spatial_shapes: torch.Tensor,
        sampling_locations: torch.Tensor,
        attention_weights: torch.Tensor) -> torch.Tensor:
