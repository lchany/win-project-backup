...
class InfoBatch(Dataset):
    ...
    def __init__(self, dataset: Dataset, num_epochs: int,
        ...
        self.scores = torch.ones(len(self.dataset)) * 5
    ...
    def update(self, values):
        ...
        batch_size = len(self.cur_batch_index)
...
class SeTa(InfoBatch):
    ...
    def prune(self):
        # Keep ranks on the same random sample set without fixing runs to a
        # hard-coded or iteration-derived seed.
