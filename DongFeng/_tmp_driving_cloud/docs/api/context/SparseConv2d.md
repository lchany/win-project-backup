...
### 接口原型
```python
mx_driving_cloud.SparseConv2d(in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, indice_key=None, mode='mmcv') -> SparseConvTensor
```
...
- Atlas A2 训练系列产品
### 约束说明
- `kernel_size`当前支持数据类型为二维List/Tuple或Int，值域为`[1, 3]`
- `stride`当前支持数据类型为二维List/Tuple或Int
...
import numpy as np
from mx_driving_cloud import SparseConv2d, SparseConvTensor
...
feature = tensor_uniform = torch.rand(actual_num, in_channels, dtype=dtype).npu()
mode = "spconv"
x = SparseConvTensor(feature, indices, spatial_shape, batch_size)
net = SparseConv2d(in_channels, out_channels, kernel_size, stride, padding, bias=False,mode=mode).npu()
