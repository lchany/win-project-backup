## cloud_dynamic_scatter
### 接口原型
```python
mx_driving_cloud.dynamic_scatter_cloud(Tensor feats, Tensor coors, str reduce_type = 'max') -> Tuple[torch.Tensor, torch.Tensor]
```
### 功能描述
将点云特征点在对应体素中进行特征压缩。
### 参数说明
...
from mx_driving_cloud import dynamic_scatter_cloud
...
voxel_feats, voxel_coors = dynamic_scatter_cloud(feats, coors, 'max')
