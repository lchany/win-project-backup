...
[[ "$active" -eq 0 ]]
set +e
timeout --signal=TERM --kill-after=5 930 docker exec \
  -e STEP215_ROOT="$root" -e OUTPUT_DIR="$output" -e MASTER_PORT="$port" \
  "$container" bash "$root/harness/step215_qr_24shape_run_inside_container.sh"
status=$?
set -e
