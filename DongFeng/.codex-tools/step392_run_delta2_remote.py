#!/usr/bin/env python3
"""Disarmed remote controller skeleton for the STEP392 diagnostic shadow run."""
...
import step392_prepare_delta2_shadow as evidence_contract
...
NPU_READY = False
REMOTE_DIAG_NAME = "step392_attempt1_qrv2_delta2_only_shadow_world8_20260822"
ATTEMPT5_DIR = "/mnt/sfs_turbo/workdir/wfc1_leicheng/diagnostics/step385_attempt5_qrv2_delta2_only_opc_build_20260822"
ATTEMPT5_MANIFEST = ATTEMPT5_DIR + "/work/release_manifest.json"
ATTEMPT5_MANIFEST_SHA256: str | None = "0221f5b64fe682d230f834554b3b8d977673f807c6a890c5279c835ebe173de8"
ATTEMPT5_RECEIPT = ATTEMPT5_DIR + "/build_receipt.json"
ATTEMPT5_RECEIPT_SHA256 = "2d5845c7c2dd74b5d50c7689e3c89f0c1144e472e804f34d8bbd421770d26f9c"
ATTEMPT5_COMPLETION = ATTEMPT5_DIR + "/work/step384_build_attempt.json"
ATTEMPT5_COMPLETION_SHA256 = "d37f0bf9754fe9e67a5428b133d396792f9237975b2e120258095730c4ba8dda"
IMMUTABLE_ORIGINAL_WHEEL: str | None = ATTEMPT5_DIR + "/work/outer_original/mx_driving_cloud-26.0.7+CANN8.3.RC1.A3-cp311-cp311-linux_aarch64.whl"
...
PROCESS_GUARD_SHA256 = "8f4886838c39f96e662ff2a5b3d17c79c9ee01d76bfe826f4b19fb63a66e8199"
EVIDENCE_SHA256 = "90926484a28fbfe7e1e69f52d5154fe2edcfeed683ad9c0e02b06ca7c9ea3fc9"
IDLE_SENTINEL_PGID = 2147483647
PORT = 34377

TOOLS_TO_UPLOAD = (
    TOOLS / "step392_prepare_delta2_shadow.py",
    TOOLS / "step392_delta2_math_worker.py",
    TOOLS / "step392_delta2_host_case.py",
...
def _upload_relative(path: Path) -> str:
    name = "step377_diagnostic_math_worker.py" if path.name == "step392_delta2_math_worker.py" else path.name
    return ("inputs/" if path.suffix == ".pt" else "") + name


EXPECTED_SHA256 = {
    "step392_prepare_delta2_shadow.py": "93c27232ac9af85a10bb3c1f97ce525e67179de9012ec63412a39e72e5d0c4a6",
    "step392_delta2_math_worker.py": "a25140870c931ecaf3113b3cab34e243a9a5953e9c2be7194086567bba9fc123",
    "step392_delta2_host_case.py": "10a6dc2efdec2d2f1a511401576e61f701ee17cc6c67d94969d227720dbfd71b",
...
FORBIDDEN_ACTIONS = (
    ...
    "train", "training", "30step", "build", "download_remote_artifacts",
)
...
from pathlib import Path
r=Path(sys.argv[1]); p=r/'shadow_manifest.json'; m=safe_json(p,4194304); source_data,_=safe_file(sys.argv[5],4194304,sys.argv[3],capture=True); source=json.loads(source_data.decode()); assert set(m)=={'schema','status','diagnostic_only','package_forbidden','source_overlay','record_unchanged','attempt5_manifest','original_wheel','shadow_root','package_root','candidate_identity','record','attempt5_artifact_inputs','artifacts'}
...
assert m['schema']=='step392-diagnostic-shadow-v1' and m['status']==sys.argv[2] and m['diagnostic_only'] is True and m['package_forbidden'] is True and m['source_overlay'] is False and m['record_unchanged'] is True
assert m['candidate_identity']=='QrV2_qa_position_delta2_only_diagnostic_v1' and m['attempt5_manifest']['sha256']==sys.argv[3] and m['original_wheel']['sha256']==sys.argv[4]
...
assert m['attempt5_manifest']['path']==str(Path(sys.argv[5]).resolve(strict=True))
assert set(m['attempt5_artifact_inputs'])=={'ascend910_93','ascend910b'} and set(source.get('artifacts',{}))=={'ascend910_93','ascend910b'}
...
for soc,v in m['artifacts'].items():
 ...
 src=source['artifacts'][soc]; inp=m['attempt5_artifact_inputs'][soc]; assert set(inp)=={'object_path','object_sha256','json_path','json_sha256'}
 ...
 for key in ('json_path','object_path'):
  q=Path(v[key]); expected=r/'shadow/mx_driving_cloud/packages/vendors/customize/op_impl/ai_core/tbe/kernel'/soc/'qr_v2'/('QrV2_qa_position_delta2_only_diagnostic_v1'+('.json' if key=='json_path' else '.o')); assert q.absolute()==expected.absolute(); bound(r/'shadow',q,v[key.replace('_path','_sha256')])
 config=r/'shadow/mx_driving_cloud/packages/vendors/customize/op_impl/ai_core/tbe/kernel/config'/soc
 qcfg=json.loads(bound(r/'shadow',config/'qr_v2.json',v['config_sha256'],True)); rows=qcfg.get('binList'); assert isinstance(rows,list) and len(rows)==1 and rows[0].get('binInfo')=={'jsonFilePath':soc+'/qr_v2/QrV2_qa_position_delta2_only_diagnostic_v1.json'}
 bcfg=json.loads(bound(r/'shadow',config/'binary_info_config.json',v['binary_info_config_sha256'],True)); binary=bcfg.get('QrV2',{}).get('binaryList'); assert isinstance(binary,list) and len(binary)==2 and all(x.get('binPath')==soc+'/qr_v2/QrV2_qa_position_delta2_only_diagnostic_v1.o' for x in binary)
...
for soc in ('ascend910_93','ascend910b'):
 ...
 newbase=prefix+soc+'/qr_v2/QrV2_qa_position_delta2_only_diagnostic_v1'; mutable|={prefix+old_json,prefix+old_object,newbase+'.json',newbase+'.o',qname,bname}
 ...
 expected_q,qcount=transform(qold,old_json,soc+'/qr_v2/QrV2_qa_position_delta2_only_diagnostic_v1.json'); expected_b,bcount=transform(bold,old_object,soc+'/qr_v2/QrV2_qa_position_delta2_only_diagnostic_v1.o')
...
from pathlib import Path
r=Path(sys.argv[1]); forbidden=json.loads(sys.argv[2]); expected={'package','wheel_write','release','install','modify_installed','train','training','30step','build','download_remote_artifacts'}; assert set(forbidden)==expected and len(forbidden)==len(expected)
def forbidden_path(p):
 name=p.name.lower(); exact={x for x in expected if x not in {'build'}}|{'release_after_npu_smi'}
 return name in exact or any(name.startswith(x+'_') for x in exact) or p.suffix in {'.whl','.zip'} or (p.is_dir() and name=='build')
bad=[str(p) for p in r.rglob('*') if forbidden_path(p)]; assert not bad; print(json.dumps({'bad':bad,'checked':sorted(expected)}))'''
...
def _validate_ownership_evidence(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != keys or not _lower_hex64(value.get("sha256")):
        raise RuntimeError("STEP392 ownership evidence mismatch")
    ...
    if not isinstance(manifest, dict) or set(manifest) != manifest_keys or manifest.get("schema") != "step358-launcher-ownership-v1" or manifest.get("port") != PORT:
        raise RuntimeError("STEP392 ownership manifest mismatch")
    if type(value["device"]) is not int or value["device"] < 0 or any(
        ...
        raise RuntimeError("STEP392 ownership identity mismatch")
    if any(type(manifest[key]) is not int or manifest[key] <= 1 for key in ("launcher_host_pid", "launcher_starttime", "launcher_pgid")):
        raise RuntimeError("STEP392 ownership process identity mismatch")
    return value
...
def _validate_rank_ownership_evidence(value: Any, launcher_sha256: str, case_path: str) -> dict[str, Any]:
    keys = {"manifest", "sha256", "device", "inode", "size", "mtime_ns", "ctime_ns"}
    if not isinstance(value, dict) or set(value) != keys or not _lower_hex64(value.get("sha256")):
        raise RuntimeError("STEP392 rank ownership evidence mismatch")
    if type(value["device"]) is not int or value["device"] < 0 or any(
        type(value[key]) is not int or value[key] <= 0 for key in ("inode", "size", "mtime_ns", "ctime_ns")
    ):
        raise RuntimeError("STEP392 rank ownership file identity mismatch")
    manifest = value.get("manifest")
    ...
    for row in ranks:
        ...
        if row["rank"] != row["local_rank"] or row["device_id"] != 8 + row["rank"] or row["physical"] != 4 + row["rank"] // 2 or row["chip"] != row["rank"] % 2 or row["host_pid"] <= 1 or row["container_pid"] <= 1 or row["starttime"] <= 0 or row["pgid"] <= 1 or any(item <= 0 for item in row["nspid"]) or row["nspid"][0] != row["host_pid"] or row["nspid"][-1] != row["container_pid"]:
            raise RuntimeError("STEP392 rank ownership identity mismatch")
    if {row["rank"] for row in ranks} != set(range(8)) or len({(row["host_pid"], row["starttime"]) for row in ranks}) != 8 or len({row["container_pid"] for row in ranks}) != 8:
        raise RuntimeError("STEP392 rank ownership bijection mismatch")
    return value
...
def _validate_stable_clear(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("STEP392 stable-clear schema mismatch")
    return value
...
def _validate_guard_result(value: Any, *, cleanup: bool, expect_rank: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != expected or value.get("schema") != schema or value.get("port_free") is not True:
        raise RuntimeError("STEP392 process guard result mismatch")
    ...
    if cleanup:
        ...
        if (
            ...
            raise RuntimeError("STEP392 cleanup guard schema mismatch")
        ...
        if expect_rank:
            if not isinstance(rank, dict) or set(rank) != {"schema", "rank_count"} or rank.get("schema") != "step377-fixed-ranks-clean-v1" or type(rank.get("rank_count")) is not int or rank.get("rank_count") != 8:
                raise RuntimeError("STEP392 rank cleanup guard schema mismatch")
        elif rank is not None:
            raise RuntimeError("STEP392 unexpected rank cleanup evidence")
    return value
...
def local_preflight() -> None:
    ...
    if NPU_READY is not True:
        raise RuntimeError("STEP392 NPU controller is intentionally disarmed")
    if not all(_lower_hex64(value) for value in (
        ATTEMPT5_MANIFEST_SHA256, IMMUTABLE_ORIGINAL_WHEEL_SHA256
    )) or not isinstance(IMMUTABLE_ORIGINAL_WHEEL, str):
        raise RuntimeError("STEP392 remote artifact contract is not armed")
    if {path.name for path in FILES} != set(EXPECTED_SHA256):
        raise RuntimeError("STEP392 upload inventory mismatch")
    evidence = evidence_contract.load_locked_evidence(EVIDENCE)
    receipt_expected = {
        "path": ATTEMPT5_RECEIPT, "dev": 51, "ino": 20158990,
        "mode": 0o600, "size": 3065, "sha256": ATTEMPT5_RECEIPT_SHA256,
    }
    if (
        evidence["manifest"]["path"] != ATTEMPT5_MANIFEST
        or evidence["manifest"]["sha256"] != ATTEMPT5_MANIFEST_SHA256
        or evidence["receipt"] != receipt_expected
        or evidence["completion"]["path"] != ATTEMPT5_COMPLETION
        or evidence["completion"]["sha256"] != ATTEMPT5_COMPLETION_SHA256
    ):
        raise RuntimeError("STEP392 attempt5 evidence constants mismatch")
    for path in FILES:
        if path.is_symlink() or not path.is_file() or sha256_file(path) != EXPECTED_SHA256[path.name]:
            raise RuntimeError(f"STEP392 local input mismatch: {path.name}")
    if REMOTE_EXEC.is_symlink() or sha256_file(REMOTE_EXEC) != REMOTE_EXEC_SHA256:
        raise RuntimeError("STEP392 remote helper mismatch")


def load_backend() -> Any:
    spec = importlib.util.spec_from_file_location("_step392_step357", STEP357)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load STEP392 remote backend")
...
class RealBackend:
    ...
    def parse_machine_info(self, _path: Path) -> dict[str, object]:
        ...
        if not target.is_private or str(target).split(".")[-1] != "42":
            raise RuntimeError("STEP392 target mapping must be private and end in 42")
        if info["jump_host"] == info["target_host"]:
            raise RuntimeError("STEP392 mapping must contain two distinct hops")
    ...
    def connect_target(self, info: dict[str, object]):
        if self.remote_module is None:
            raise RuntimeError("STEP392 remote module was not loaded")
    ...
    def require_hostname(self, target: Any) -> None:
        ...
        if out.strip() != self.legacy.EXPECTED_HOSTNAME:
            raise RuntimeError("STEP392 target hostname mismatch")

    def require_container(self, target: Any, name: str) -> None:
        if name != CONTAINER:
            raise RuntimeError("STEP392 container mismatch")
    ...
    def exclusive_directory(self, target: Any, name: str, mode: int) -> None:
        if name != REMOTE_DIAG_NAME or mode != 0o700 or self.remote_root is None:
            raise RuntimeError("STEP392 remote directory contract mismatch")
        script = (
    ...
    def upload_new(self, _target: Any, path: Path, destination: str, digest: str) -> None:
        if self.sftp is None:
            raise RuntimeError("STEP392 upload precondition mismatch")
        payload, actual = read_local_regular(path, digest)
        if actual != digest:
            raise RuntimeError("STEP392 upload precondition mismatch")
        relative = _upload_relative(path)
        if self.remote_root is None or destination != REMOTE_DIAG_NAME + "/" + relative:
            raise RuntimeError("STEP392 upload destination mismatch")
    ...
    def verify_uploads(self, target: Any, *, pre_host: bool = False) -> None:
        if self.remote_root is None:
            raise RuntimeError("STEP392 remote root unavailable")
        expected = {
            _upload_relative(path): EXPECTED_SHA256[path.name]
            for path in FILES
        }
        ...
        if value != {"count": len(expected)}:
            raise RuntimeError("STEP392 remote upload inventory mismatch")
    ...
    def snapshot(self, target: Any, artifact: dict[str, Any]) -> dict[str, Any]:
        ...
        if (not isinstance(artifacts, dict) or not isinstance(installed, dict)
            ...
            raise RuntimeError("STEP392 snapshot schema mismatch")
        return {
            "schema": "step392-protected-snapshot-v1",
            "artifacts": artifacts,
            "runtime": runtime,
    ...
    def require_stable_clear(self, target: Any) -> dict[str, Any]:
        ...
        command = (
            ...
            + shlex.quote(self.remote_root + "/step392_delta2_host_case.py")
    ...
    def prepare_shadow(self, target: Any, plan: dict[str, Any], timeout: int) -> None:
        ...
        command = (
            ...
            + " python3 " + shlex.quote(self.remote_root + "/step392_prepare_delta2_shadow.py")
            + " --attempt5-manifest " + shlex.quote(ATTEMPT5_MANIFEST)
            ...
            + " --evidence " + shlex.quote(self.remote_root + "/STEP392_attempt5_evidence.json")
        )
        self.legacy.run(target, command, timeout=timeout)

    def verify_attempt5_evidence(self, target: Any) -> None:
        if self.remote_root is None:
            raise RuntimeError("STEP392 remote root unavailable")
        code = (
            "import json,sys;sys.path.insert(0,sys.argv[1]);"
            "import step392_prepare_delta2_shadow as m;"
            "v=m.load_locked_evidence(__import__('pathlib').Path(sys.argv[2]));"
            "m.verify_live_evidence(v);print(json.dumps({'status':'evidence_closed'}))"
        )
        command = (
            "docker exec -e PYTHONDONTWRITEBYTECODE=1 " + shlex.quote(CONTAINER)
            + " python3 -c " + shlex.quote(code)
            + " " + shlex.quote(self.remote_root)
            + " " + shlex.quote(self.remote_root + "/STEP392_attempt5_evidence.json")
        )
        out, _ = self.legacy.run_host_script(target, command, timeout=120)
        value = json.loads(out)
        if value != {"status": "evidence_closed"}:
            raise RuntimeError("STEP392 live evidence closure mismatch")

    def validate_shadow(self, target: Any, **contract: Any) -> None:
        if self.remote_root is None:
            raise RuntimeError("STEP392 remote root unavailable")
        code = shadow_embedded_script()
        self._run_json(target, code, self.remote_root + "/shadow_work", contract["status"], str(ATTEMPT5_MANIFEST_SHA256), str(IMMUTABLE_ORIGINAL_WHEEL_SHA256), ATTEMPT5_MANIFEST, str(IMMUTABLE_ORIGINAL_WHEEL))
    ...
    def pre_host_closure(self, target: Any, artifact: dict[str, Any]) -> None:
        ...
        self.verify_attempt5_evidence(target)
        current = self.validate_readonly_artifacts(
            target, ATTEMPT5_MANIFEST, ATTEMPT5_MANIFEST_SHA256,
            IMMUTABLE_ORIGINAL_WHEEL, IMMUTABLE_ORIGINAL_WHEEL_SHA256,
        )
        if current != artifact:
            raise RuntimeError("STEP392 pre-host artifact identity changed")
    ...
    def run_host_once(self, target: Any, plan: dict[str, Any], **contract: Any) -> dict[str, Any]:
        ...
        if (self.remote_root is None
            ...
            raise RuntimeError("STEP392 host launch contract mismatch")
        command = (
            "cd " + shlex.quote(self.remote_root) + " && PYTHONDONTWRITEBYTECODE=1 python3 step392_delta2_host_case.py"
    ...
    def read_rank_ownership(self, target: Any, launcher_sha256: str) -> dict[str, Any] | None:
        if self.remote_root is None or not _lower_hex64(launcher_sha256):
            raise RuntimeError("STEP392 rank ownership precondition mismatch")
        ...
        return _validate_rank_ownership_evidence(
            ...
            self.remote_root + "/step392_delta2_host_case.py",
        )

    def read_summary(self, target: Any, name: str) -> dict[str, Any]:
        if self.remote_root is None or name != REMOTE_DIAG_NAME:
            raise RuntimeError("STEP392 summary path mismatch")
        code = summary_embedded_script()
        return self._run_json(target, code, self.remote_root + "/run/step392_diagnostic_summary.json")

    def scan_forbidden_outputs(self, target: Any, name: str, forbidden: tuple[str, ...]) -> None:
        if self.remote_root is None or name != REMOTE_DIAG_NAME:
            raise RuntimeError("STEP392 forbidden scan path mismatch")
        code = forbidden_embedded_script()
        self._run_json(target, code, self.remote_root, json.dumps(list(forbidden)))
    ...
    def cleanup_owned(self, target: Any, port: int, ownership: dict[str, Any], rank_ownership: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if (
            ...
            raise RuntimeError("STEP392 cleanup ownership contract mismatch")
        if self.remote_root is None:
            raise RuntimeError("STEP392 remote root unavailable")
        ...
        command = (
            ...
            + " --case-path " + shlex.quote(self.remote_root + "/step392_delta2_host_case.py")
            + " --port " + str(port)
        )
        if rank_ownership is not None:
            command += (
...
def dry_run_plan() -> dict[str, Any]:
    local_preflight()
    return {
        "schema": "step392-diagnostic-remote-plan-v1",
        "diagnostic_only": True,
        "container": CONTAINER,
        "remote_directory": REMOTE_DIAG_NAME,
        "attempt5_manifest": ATTEMPT5_MANIFEST,
...
def validate_summary(value: Any) -> dict[str, Any]:
    keys = {
        ...
        "ranks", "raw_profiles", "candidate_identity", "concrete_aic_hit_count",
        "concrete_aiv_hit_count", "forbidden_identity_hit_count", "math_gate",
    }
    if not isinstance(value, dict) or set(value) != keys:
        raise RuntimeError("STEP392 remote summary schema mismatch")
    if (
        value["schema"] != "step392-diagnostic-host-summary-v1"
        ...
        or value["candidate_identity"] != "QrV2_qa_position_delta2_only_diagnostic_v1"
        or type(value["concrete_aic_hit_count"]) is not int
        or value["concrete_aic_hit_count"] != 8
        or type(value["concrete_aiv_hit_count"]) is not int
        or value["concrete_aiv_hit_count"] != 0
        or type(value["forbidden_identity_hit_count"]) is not int
        or value["forbidden_identity_hit_count"] != 0
        or value["math_gate"] != {
            "input_unmodified": True, "input_finite": True, "q_finite": True,
            "r_finite": True, "reconstruction": True, "orthogonality": True,
            "lower_triangle_exact_zero": True, "cpu_qr_projection": True,
        }
        ...
        raise RuntimeError("STEP392 remote summary contract mismatch")
    if value["input_sha256"] != EXPECTED_INPUT_SHA256:
        raise RuntimeError("STEP392 remote summary input closure mismatch")
    if set(value["module_file_sha256"]) != {"cloud_init", "cloud_extension", "cloud_linalg"} or not all(
        ...
        raise RuntimeError("STEP392 remote summary module closure mismatch")
    ranks = value["ranks"]
    if any(
        not isinstance(row, dict) or set(row) != {
            "rank", "call_count", "identity_pass", "concrete_aic_hits",
            "concrete_aiv_hits", "original_hits", "v4_hits", "v5_hits",
            "legacy_v6_hits", "unknown_hits",
        }
        or type(row["rank"]) is not int or type(row["call_count"]) is not int
        or row["call_count"] != 1 or row["identity_pass"] is not True
        or type(row["concrete_aic_hits"]) is not int or row["concrete_aic_hits"] != 1
        ...
        raise RuntimeError("STEP392 remote summary rank closure mismatch")
    ...
    if any(
        ...
        raise RuntimeError("STEP392 remote raw profile closure mismatch")
    return value
...
def _strict_absolute_path(value: Any, label: str) -> str:
    if (not isinstance(value, str) or not value.startswith("/") or value == "/"
        ...
        raise RuntimeError(f"STEP392 {label} path contract mismatch")
    return value


def _installed_root_from_runtime(runtime: Any) -> str:
    if not isinstance(runtime, dict):
        raise RuntimeError("STEP392 runtime schema mismatch")
...
def _validate_snapshot(value: Any) -> dict[str, Any]:
    if (
        ...
        or value.get("schema") != "step392-protected-snapshot-v1"
        ...
        raise RuntimeError("STEP392 protected snapshot mismatch")
    return value
...
def execute(backend: Any | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        remote.verify_attempt5_evidence(target)
        ...
        if result["launcher_ownership_sha256"] != ownership["sha256"] or rank_ownership is None or result["rank_ownership_sha256"] != rank_ownership["sha256"] or result["gate_token_sha256"] != rank_ownership["manifest"]["gate_token_sha256"]:
            raise RuntimeError("STEP392 summary ownership closure mismatch")
        ...
        if owned and target is not None and artifact is not None and before is not None:
            try:
                ...
                if before != after:
                    raise RuntimeError("STEP392 installed/runtime/artifact closure changed")
    ...
    if primary is not None:
        raise primary
    if result is None:
        raise RuntimeError("STEP392 transaction returned no summary")
    if cleanup_postflight is None:
        raise RuntimeError("STEP392 cleanup returned no postflight")
    result = dict(result)
    result["cleanup_postflight"] = {
        "schema": "step392-controller-cleanup-postflight-v1",
