...
FILES = ("step307_launch_inside.sh",)
...
def main() -> int:
    ...
    try:
        ...
        precheck = (
            ...
            "! ss -ltn | awk '{print $4}' | grep -Eq ':(30162|6422[0-9]|6423[0-9])$' && "
        ...
        try:
            ...
            sftp.close()
        command = (
            f"chmod 755 {remote_dir}/step307_launch_inside.sh; "
            f"docker exec mapqr-leicheng bash -n {remote_dir}/step307_launch_inside.sh && "
            f"setsid -f sh -c 'docker exec -e STEP307_OUT={remote_dir} -e HCCL_IF_BASE_PORT=64220 "
            f"mapqr-leicheng bash --noprofile --norc {remote_dir}/step307_launch_inside.sh' </dev/null; echo started"
        )
