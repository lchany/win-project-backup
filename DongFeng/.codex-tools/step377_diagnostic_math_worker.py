...
GATE_SCHEMA = "step377-diagnostic-start-v1"
ACK_SCHEMA = "step377-diagnostic-gate-ack-v1"
...
def _guard_dependencies() -> None:
    for path in (WORKER_PATH, COLD_CASE_PATH, ORACLE_PATH):
        if path.is_symlink() or not path.is_file():
            raise RuntimeError(f"STEP377 dependency must be regular: {path.name}")
        if sha256_file(path) != EXPECTED_SHA256[path.name]:
            raise RuntimeError(f"STEP377 dependency SHA mismatch: {path.name}")


def _load_module(name: str, path: Path) -> ModuleType:
    spec = importlib.util.spec_from_file_location(name, path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"unable to load STEP377 dependency: {path.name}")
...
def load_worker() -> ModuleType:
    _guard_dependencies()
    return _load_module("_step377_isolated_step358_worker", WORKER_PATH)
...
def _referenced_qrv2(worker: ModuleType, profile_root: Path) -> dict[str, int]:
    ...
    if duplicate:
        raise RuntimeError(
            "STEP377 referenced QrV2 identity maps to multiple hashes: "
...
def install_worker_identity(worker: ModuleType) -> Callable[[], None]:
    ...
    def verify(profile_root: Path, *, expected_aic_references: int) -> dict[str, Any]:
        if expected_aic_references != 1:
            raise RuntimeError("STEP377 requires exactly one diagnostic task reference")
        ...
        if referenced != {DIAGNOSTIC_AIC: 1}:
            raise RuntimeError(
                "STEP377 task-referenced QrV2 identity set is not exact: "
...
def install_diagnostic_wait(cold_case: ModuleType) -> Callable[[], None]:
    ...
    def wait(path: Path, timeout_seconds: int = 120) -> None:
        if path.name != LEGACY_START_NAME:
            raise RuntimeError("STEP377 worker received an unexpected start gate")
        ...
        if rank_text not in {str(rank) for rank in range(8)}:
            raise RuntimeError("STEP377 LOCAL_RANK is invalid for gate acknowledgement")
...
def run(args: argparse.Namespace) -> int:
    _guard_dependencies()
    cold_case = _load_module("_step377_isolated_cold_case", COLD_CASE_PATH)
    worker = load_worker()
    ...
    if primary is not None:
        raise primary
    if result is None:
        raise RuntimeError("STEP377 underlying worker returned no result")
    return result
