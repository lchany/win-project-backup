...
OUT=${STEP332_OUT:?STEP332_OUT is required}
...
OVERLAY="${STEP332_SOAP_OVERLAY:?STEP332_SOAP_OVERLAY is required}"
if [ ! -f "$OVERLAY" ]; then
...
{
  echo "STEP332_START $(date -Iseconds)"
  ...
  grep -n "n_query_false\|_install_diag_log\|torch.linalg.qr" projects/mmdet3d_plugin/optimizers/soap.py | head -8 || true
  tmp=$(mktemp)
...
echo "STEP332_END rc=$rc $(date -Iseconds)" >>"$OUT/logs/launcher.log"
exit $rc
