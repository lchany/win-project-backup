...
OUT=${STEP303_OUT:?STEP303_OUT is required}
...
echo "STEP303_START $(date -Iseconds)"
...
python -m torch.distributed.launch --nproc_per_node=8 --master_port=30141 --use_env \
...
p = Path(os.environ["STEP303_OUT"]) / "logs" / "device_preflight.log"
rows = []
...
echo "STEP303_END rc=$rc $(date -Iseconds)"
exit "$rc"
