...
REL = "diagnostics/step308_head27b1d6d_direct30step_nohcclbase_20260819T172300"
FILES = ("step308_launch_inside.sh",)
...
def main() -> int:
    ...
    try:
        ...
        precheck = (
            ...
            "! ss -ltn | awk '{print $4}' | grep -Eq ':(30172)$' && "
        ...
        command = (
            ...
            f"p = Path('{remote_dir}/step308_launch_inside.sh')\n"
            f"p.write_bytes(p.read_bytes().replace(b'\\r\\n', b'\\n'))\n"
            f"PY\n"
            f"chmod 755 {remote_dir}/step308_launch_inside.sh; "
            f"docker exec mapqr-leicheng bash -n {remote_dir}/step308_launch_inside.sh && "
            f"setsid -f sh -c 'docker exec -e STEP308_OUT={remote_dir} "
            f"mapqr-leicheng bash --noprofile --norc {remote_dir}/step308_launch_inside.sh' </dev/null; echo started"
        )
