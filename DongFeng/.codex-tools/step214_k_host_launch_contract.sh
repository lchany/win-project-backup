...
set +e;timeout --signal=TERM --kill-after=5 120 docker exec -e STEP214_ROOT="$r" -e OUTPUT_DIR="$o" -e MASTER_PORT="$p" "$c" bash "$r/harness/step214_k_run_inside_container.sh";x=$?;set -e
