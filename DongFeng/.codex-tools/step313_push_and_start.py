...
REL = "diagnostics/step313_qr_dump_nan_iter6_back8_20260819T175500"
...
def main() -> int:
    ...
    try:
        ...
        command = (
            ...
            f"'''STEP-313 QR dump capture for back-8 training NaN investigation.\n"
            f"Purpose: capture QR input/output tensors around the first training NaN (observed from iter 6 onward).\n"
            f"Environment: mapqr-leicheng, ASCEND_RT_VISIBLE_DEVICES=8,9,10,11,12,13,14,15, official mx_driving_cloud.linalg.qr, no bypass.\n"
            f"Expected contents:\n"
            f"- logs/launcher.log : 10-step training log\n"
            f"- qr_tensors/ : QR SAMPLE/BAD tensors saved by SOAP_QR_DUMP_DIR\n"
            f"- launcher_rc.txt : launcher exit code\n"
