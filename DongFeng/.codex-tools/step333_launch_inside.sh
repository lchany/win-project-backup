...
OUT=${STEP333_OUT:?STEP333_OUT is required}
BACK8_DEVICES="8,9,10,11,12,13,14,15"
SOAP_K=${SOAP_STALE_Q_K:-4}
...
export SOAP_INSTALL_DIAG=1
export SOAP_INSTALL_DIAG_LOG="$OUT/logs/install_diag.jsonl"
...
OVERLAY="${STEP333_SOAP_OVERLAY:?STEP333_SOAP_OVERLAY is required}"
if [ ! -f "$OVERLAY" ]; then
  echo "OVERLAY_MISSING $OVERLAY"
  exit 94
fi
cp "$OVERLAY" "$REPO/projects/mmdet3d_plugin/optimizers/soap.py"

test -f "$OUT/step331_cycle_profiler_hook.py"
export STEP331_REPO="$REPO"
python3 "$OUT/step331_build_profile_config.py" --repo "$REPO" "$REPO/$BASE_CONFIG" "$OUT/step333_profile_config.py"

{
  echo "STEP333_START $(date -Iseconds)"
  echo "HEAD=$(git rev-parse HEAD)"
  echo "SOAP_STALE_Q_K=$SOAP_STALE_Q_K SOAP_INSTALL_DIAG=$SOAP_INSTALL_DIAG"
  ...
  echo "SOAP_OVERLAY_SHA=$(sha256sum "$OVERLAY" | awk '{print $1}')"
  grep -n "_install_diag_log\|stale_q_install_if_due\|n_query_false" projects/mmdet3d_plugin/optimizers/soap.py | head -6 || true
  tmp=$(mktemp)
  sed "s/^export SOAP_STALE_Q_K=.*/export SOAP_STALE_Q_K=${SOAP_K}/" tools/ddp_train.sh >"$tmp"
  bash "$tmp" tools/train_spetr.py "$OUT/step333_profile_config.py"
  rm -f "$tmp"
} >"$OUT/logs/launcher.log" 2>&1
rc=$?
echo "$rc" >"$OUT/launcher_rc.txt"
echo "STEP333_END rc=$rc $(date -Iseconds)" >>"$OUT/logs/launcher.log"
exit $rc
