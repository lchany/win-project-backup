...
[[ "$o" == "$r"/runs/step214_l_forward_* && ! -e "$o" ]];h="$r/harness/step214_l_msda_forward_2head_gate.py";c="$r/harness/step214_j_ready_controller.py";mkdir -p "$o";exec > >(tee "$o/wrapper.log") 2>&1
echo contract=step214_l_msda_forward_b1_twohead_world8;date -u +start_utc=%FT%TZ;sha256sum "$h" "$r/harness/step214_k_msda_forward_gate.py" "$c" > "$o/source_sha256.txt"
...
unset TRITON_ALL_BLOCKS_PARALLEL || true
...
[[ "$(find "$o/done" -name 'rank*.json'|wc -l)" -eq 8 ]];find "$o" -type f -print0|sort -z|xargs -0 sha256sum > "$o/artifacts_sha256.txt";date -u +end_utc=%FT%TZ;echo decision=PASS_MSDA_FORWARD_B1_TWOHEAD
