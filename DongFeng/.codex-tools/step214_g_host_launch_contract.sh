...
if [[ "$#" -ne 3 ]]; then
  echo "usage: $0 STEP214_ROOT OUTPUT_DIR MASTER_PORT" >&2
  exit 2
fi
step214_root=$1
output_dir=$2
...
[[ "$(readlink -m -- "$output_dir")" == "$(readlink -f -- "$step214_root")"/runs/step214_g_* ]]
[[ ! -e "$output_dir" ]]
...
timeout --signal=TERM --kill-after=5 120 docker exec \
  -e STEP214_ROOT="$step214_root" \
  -e OUTPUT_DIR="$output_dir" \
  -e MASTER_PORT="$master_port" \
  "$container" \
  bash "$step214_root/harness/step214_g_run_inside_container.sh"
rc=$?
set -e
if [[ "$rc" -eq 124 || "$rc" -eq 137 ]]; then
  docker exec "$container" bash -c \
    "touch '$output_dir/release_after_npu_smi' 2>/dev/null || true; pkill -TERM -f '$output_dir' 2>/dev/null || true"
fi
exit "$rc"
