...
# The trusted STEP377 pidfd guard intentionally authorizes this immutable
# runtime basename.  The STEP392 controller uploads the locked STEP392 bytes
# under that private staging alias; the historical local file is untouched.
...
EXPECTED_SHA256 = {
    ...
    WORKER_ADAPTER.name: "a25140870c931ecaf3113b3cab34e243a9a5953e9c2be7194086567bba9fc123",
    PROCESS_GUARD_PATH.name: "8f4886838c39f96e662ff2a5b3d17c79c9ee01d76bfe826f4b19fb63a66e8199",
}
RANKS = tuple(range(8))
...
SUMMARY_NAME = "step392_diagnostic_summary.json"
READY_KEYS = {
...
def _guard_dependencies() -> None:
    for path in (HOST_PATH, WORLD8_PATH, WORKER_ADAPTER, PROCESS_GUARD_PATH):
        if path.is_symlink() or not path.is_file():
            raise RuntimeError(f"STEP392 host dependency must be regular: {path.name}")
        if sha256_file(path) != EXPECTED_SHA256[path.name]:
            raise RuntimeError(f"STEP392 host dependency SHA mismatch: {path.name}")
...
def load_host() -> ModuleType:
    _guard_dependencies()
    world = _exec_module("_step392_isolated_world8", WORLD8_PATH)
    ...
    try:
        return _exec_module("_step392_isolated_step358_host", HOST_PATH)
    finally:
        if existed:
...
def load_process_guard() -> ModuleType:
    _guard_dependencies()
    return _exec_module("_step392_isolated_process_guard", PROCESS_GUARD_PATH)
...
def validate_outputs(root: Path, input_hashes: dict[int, str]) -> dict[str, Any]:
    ...
    for rank in RANKS:
        ...
        if (
            identity.get("candidate_aic_reference_count") != 1
            or identity.get("expected_aic_reference_count") != 1
            or identity.get("referenced_qrv2_entries")
            != ["QrV2_qa_position_delta2_only_diagnostic_v1_0_mix_aic"]
        ):
            raise RuntimeError(f"rank{rank} referenced QrV2 closure mismatch")
        ...
        rank_summaries.append({
            ...
            "concrete_aic_hits": 1, "concrete_aiv_hits": 0,
            "original_hits": 0, "v4_hits": 0, "v5_hits": 0,
            "legacy_v6_hits": 0, "unknown_hits": 0,
        })
    return {
        "ranks": rank_summaries,
        "raw_profiles": profile_inventory,
        "candidate_identity": "QrV2_qa_position_delta2_only_diagnostic_v1",
        "concrete_aic_hit_count": 8,
        "concrete_aiv_hit_count": 0,
        "forbidden_identity_hit_count": 0,
        "math_gate": {
...
def _verify_gate(path: Path, contract: dict[str, Any]) -> None:
    ...
    if (
        value.get("schema") != "step392-diagnostic-start-v1"
...
def _prepare_gate(path: Path) -> tuple[dict[str, Any], bytes]:
    ...
    payload = (json.dumps({"schema": "step392-diagnostic-start-v1", "token": token}) + "\n").encode()
...
def _validate_gate_acks(root: Path, gate: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for rank in RANKS:
        ...
        if (
            set(row) != ACK_KEYS
            or row.get("schema") != "step392-diagnostic-gate-ack-v1"
...
def diagnostic_wait_for_results(host: ModuleType, guard: ModuleType, root: Path,
    ...
    host._step392_rank_ownership = rank_ownership
    host._step392_rank_ownership_sha256 = sha256_file(rank_path)
    guard.read_rank_ownership_json(
        rank_path, host._step392_rank_ownership_sha256,
        expected_launcher_sha256=launcher_sha256,
        case_path=Path(__file__).resolve(), port=int(host._step392_port))
    gate = _publish_gate(root / GATE_NAME, prepared_gate, gate_payload)
    host._step392_gate_token_sha256 = gate["token_sha256"]
    durability_errors = []
    ...
    while True:
        ...
        time.sleep(0.25)
    controller = {
        "schema": "step392-diagnostic-world8-v1",
        ...
        "launcher_ownership_sha256": launcher_sha256,
        "rank_ownership_sha256": host._step392_rank_ownership_sha256,
    }
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        def cleanup_all(root: Path, port: int) -> None:
            ...
            rank_path = root / "rank_ownership.json"
            try:
                if rank_path.exists() and hasattr(host, "_step392_rank_ownership_sha256"):
                    ...
                    if hasattr(host, "_step392_gate_token_sha256") and rank["gate_token_sha256"] != host._step392_gate_token_sha256:
    ...
    if result_code == 0:
        ...
        if (
            controller.get("schema") != "step392-diagnostic-world8-v1"
        ...
        summary = {
            "schema": "step392-diagnostic-host-summary-v1",
