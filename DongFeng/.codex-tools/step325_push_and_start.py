...
def main() -> int:
    ...
    try:
        ...
        command = (
            ...
            f"'''STEP-325 torch backend stabilization run.\n"
            f"Purpose: extend STEP-324 control from 10 to 30 iters with SOAP_QR_BACKEND=torch.\n"
            f"Expected: no seg-loss NaN after Iter 6 and stable finite loss through Iter 30.\n"
            f"''', encoding='utf-8')\n"
            f"PY\n"
            f"chmod 755 {remote_dir}/step313_launch_inside.sh; "
            f"docker exec mapqr-leicheng bash -n {remote_dir}/step313_launch_inside.sh && "
            f"setsid -f sh -c 'docker exec -e STEP313_OUT={remote_dir} -e SOAP_QR_BACKEND=torch -e SOAP_QR_DUMP_MAX_CALLS=4 -e MAX_ITERS=30 "
