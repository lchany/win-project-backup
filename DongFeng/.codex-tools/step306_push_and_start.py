...
REL = "diagnostics/step306_head27b1d6d_setdevice_30step_20260819T171000"
FILES = ("step303_rank_device_preflight.py", "step306_launch_inside.sh")
...
def main() -> int:
    ...
    try:
        ...
        precheck = (
            ...
            "! ss -ltn | awk '{print $4}' | grep -Eq ':(30151|30152|6412[0-9]|6413[0-9])$' && "
        ...
        try:
            ...
            sftp.close()
        command = (
            f"chmod 755 {remote_dir}/step306_launch_inside.sh; "
            f"docker exec mapqr-leicheng bash -n {remote_dir}/step306_launch_inside.sh && "
            f"setsid -f sh -c 'docker exec -e STEP306_OUT={remote_dir} -e HCCL_IF_BASE_PORT=64120 "
            f"mapqr-leicheng bash --noprofile --norc {remote_dir}/step306_launch_inside.sh' </dev/null; echo started"
        )
