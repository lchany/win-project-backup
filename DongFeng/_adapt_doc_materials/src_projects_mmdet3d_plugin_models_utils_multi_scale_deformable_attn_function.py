...
class MultiScaleDeformableAttnFunction_fp32(Function):
    ...
    #              im2col_step: int):
    #     if hasattr(im2col_step, 'item'):
    #         im2col_step = int(im2col_step.item())  # Tensor -> int
    #     return g.op("trt::MultiscaleDeformableAttnPlugin_TRT", value,
    #                 value_spatial_shapes,
    #                 value_level_start_index,
    #                 sampling_locations,
    #                 attention_weights,
    ...
    def symbolic(g, value: torch.Tensor, value_spatial_shapes: torch.Tensor,
        ...
        if hasattr(im2col_step, 'item'):
            im2col_step = int(im2col_step.item())  # Tensor -> int
        # # 强制转换成 int32
        value_spatial_shapes = g.op("Cast", value_spatial_shapes, to_i=6)  # 6 = INT32
        value_level_start_index = g.op("Cast", value_level_start_index, to_i=6)
        return g.op("trt::MultiscaleDeformableAttnPlugin_TRT", value,
                    value_spatial_shapes,
                    value_level_start_index,
                    sampling_locations,
